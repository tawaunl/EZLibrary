...
APP_BUNDLE_OVERRIDE="${EZLIBRARY_APP_PATH:-${SERATOTOOLS_APP_PATH:-}}"
...
if [[ -n "$APP_BUNDLE_OVERRIDE" ]]; then
  ...
  INFERRED_BUNDLE="$(cd "$SCRIPT_DIR/../../.." && pwd)"
...
SCRIPT_PATH="$APP_RESOURCES/scripts/finder-add-music.sh"
CLI_PATH="$APP_RESOURCES/bin/EZLibraryCLI"
...
export EZLIBRARY_CLI_PATH="${CLI_PATH}"
